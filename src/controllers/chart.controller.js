...
const DashboardModel = require('../models/dashboad.model')
...
class ChartController {
    ...
    async saveDashboardIntoDataBase(req,res){
        try{   
             const {attribut1 , attribut2,fileName ,idUser ,typeOfDashboard} = req.body
        const chart = await this.drawSimple(req,res)
        const userId = req.infos.authId;
        const dashboardModel = new DashboardModel({fileName,attribut1,attribut2 ,userId,typeOfDashboard})
        await dashboardModel.save()
       
        return   res.status(StatusCodes.CREATED).json("Dashboard saved successfully")}
